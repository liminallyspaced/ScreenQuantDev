...
def main():
    ...
    check("0.0.72-slice2bs" in hello_c, "hello.c version string 0.0.72-slice2bs")
    ...
    section("Slice 2u Specular Tint / Thin Film / Subsurface TEX_IMAGE ABI")
    hdr = _read("native/quanttrace/src/quanttrace.h")
    check("spec_tint_image_path" in hdr, "QT_Mesh has spec_tint_image_path")
    check("film_thick_image_path" in hdr, "QT_Mesh has film_thick_image_path")
    check("film_ior_image_path" in hdr, "QT_Mesh has film_ior_image_path")
    check("sss_weight_image_path" in hdr, "QT_Mesh has sss_weight_image_path")
    check("sss_radius_image_path" in hdr, "QT_Mesh has sss_radius_image_path")
    check("sss_scale_image_path" in hdr, "QT_Mesh has sss_scale_image_path")
    bridge = _read("native/quanttrace/src/session_bridge.cpp")
    check('bsdf->input("Specular Tint")' in bridge, "bridge connects Specular Tint")
    check('bsdf->input("Thin Film Thickness")' in bridge, "bridge connects Thin Film Thickness")
    check('bsdf->input("Thin Film IOR")' in bridge, "bridge connects Thin Film IOR")
    check('bsdf->input("Subsurface Weight")' in bridge, "bridge connects Subsurface Weight")
    check('bsdf->input("Subsurface Radius")' in bridge, "bridge connects Subsurface Radius")
    check('bsdf->input("Subsurface Scale")' in bridge, "bridge connects Subsurface Scale")
    ...
    check("Thin Film IOR" in sync_src, "sync accepts Thin Film IOR")
    check("Subsurface Weight" in sync_src, "sync accepts Subsurface Weight")
    check("Subsurface Radius" in sync_src, "sync accepts Subsurface Radius")
    check("Subsurface Scale" in sync_src, "sync accepts Subsurface Scale")
    check("spec_tint_image_path" in sync_src and "sss_scale_image_path" in sync_src,
          "sync packs spec_tint_/sss_scale_ TEX_IMAGE fields")
    utools = _read("tools/_quanttrace_slice2u_scene.py")
    check("SpecTint" in utools and "SSSScale" in utools, "slice2u scene wires sockets")
    ...
    hello = _read("native/quanttrace/src/hello.c")
    check("0.0.72-slice2bs" in hello, "hello version is 0.0.72-slice2bs")
    ...
    hello = _read("native/quanttrace/src/hello.c")
    check("0.0.72-slice2bs" in hello, "native version 0.0.72-slice2bs")
    ...
    section("Slice 2bs ColorRamp.Fac<-MATH")
    hdr = _read("native/quanttrace/src/quanttrace.h")
    check("mix_nested2_fac" in hdr, "ABI has mix_nested2_fac")
    check("mix_nested2_ramp_enable" in hdr, "ABI has mix_nested2_ramp_enable")
    check("mix_nested2_ramp_n" in hdr, "ABI has mix_nested2_ramp_n")
    check("mix_nested2_ramp_fac" in hdr, "ABI has mix_nested2_ramp_fac")
    check("mix_nested2_ramp_math_enable" in hdr, "ABI has mix_nested2_ramp_math_enable")
    check("QT_MATH_IN_GEOM" in hdr, "ABI has QT_MATH_IN_GEOM")
    check("QT_MATH_IN_HUESAT" in hdr, "ABI has QT_MATH_IN_HUESAT")
    hello = _read("native/quanttrace/src/hello.c")
    check("0.0.72-slice2bs" in hello, "native version 0.0.72-slice2bs")
    sync_src = _read("scenequant/quanttrace/sync.py")
    check("_pack_nested2_mix_shader" in sync_src, "sync has nested2 packer")
    check("Slice 2br" in sync_src, "sync names Slice 2br refuse")
    check("mix_nested2_ramp_enable" in sync_src, "sync packs nested2 ramp")
    ident = sync._mix_nested2_identity()
    check(ident["mix_nested2_lightpath_enable"] == 0, "nested2 identity enable=0")
    check(ident["mix_nested2_ramp_enable"] == 0, "nested2 identity ramp_enable=0")
    check(int(ident["mix_nested2_ramp_n"]) == 0, "nested2 identity ramp_n=0")
    check(abs(float(ident["mix_nested2_fac"]) - 0.5) < 1e-9, "nested2 identity fac=0.5")
    check(abs(float(ident["mix_nested2_ramp_fac"]) - 0.5) < 1e-9, "nested2 identity ramp_fac=0.5")
    check(int(ident["mix_nested2_closure1_kind"]) == 0, "nested2 identity c1=Glass")
    check(int(ident["mix_nested2_closure2_kind"]) == 1, "nested2 identity c2=Transparent")
    nest = sync._mix_nested_identity()
    check(int(nest["mix_nested_closure1_kind"]) == 0, "nested identity skip nested2 (kinds 0/1)")
    check(int(nest["mix_nested2_ramp_enable"]) == 0, "nested identity includes ramp enable 0")
    check(int(ident["mix_nested2_ramp_math_enable"]) == 0, "nested2 identity ramp_math_enable=0")
    check(abs(float(ident["mix_nested2_ramp_hsv_val"]) - 1.0) < 1e-9, "nested2 identity hsv val=1")
    check("Slice 2bs" in sync_src, "sync names Slice 2bs refuse")
    check("mix_nested2_ramp_math_enable" in sync_src, "sync packs ramp math")
    check("_pack_ramp_math_node" in sync_src, "sync has ramp math packer")
    bsscene = _read("tools/_quanttrace_slice2bs_scene.py")
    check("refuse_noise" in bsscene, "2bs scene has Noise refuse")
    check("0.3874170780181885" in bsscene, "2bs CLAIM loft ColorRamp stop 0.387")
    check("ShaderNodeNewGeometry" in bsscene, "2bs CLAIM has NEW_GEOMETRY Backfacing")
    check("Ray Length" in bsscene, "2bs CLAIM HueSat Color<-Ray Length")
    check("math_fac=True" in bsscene, "2bs CLAIM wires ColorRamp.Fac MATH")
    bssmoke = _read("tools/_quanttrace_slice2bs_smoke.py")
    check("pack_scene" in bssmoke and "render_qt_scene_rgba" in bssmoke, "2bs smoke packs QT_Scene")
    check("refuse_noise" in bssmoke and "Slice 2bs" in bssmoke, "2bs smoke expects Noise refuse")
    brscene = _read("tools/_quanttrace_slice2br_scene.py")
    check("refuse_noise" in brscene, "2br scene has Noise refuse")
    check("0.3874170780181885" in brscene, "2br CLAIM loft ColorRamp stop 0.387")
    check("nested2" in brscene, "2br scene has nested2 identity mode")
    brsmoke = _read("tools/_quanttrace_slice2br_smoke.py")
    check("pack_scene" in brsmoke and "render_qt_scene_rgba" in brsmoke, "2br smoke packs QT_Scene")
    check("refuse_noise" in brsmoke and "Slice 2br" in brsmoke, "2br smoke expects Noise refuse")
