...
def main():
    ...
    check("0.0.70-slice2bq" in hello_c, "hello.c version string 0.0.70-slice2bq")
    ...
    hello = _read("native/quanttrace/src/hello.c")
    check("0.0.70-slice2bq" in hello, "hello version is 0.0.70-slice2bq")
    ...
    hello = _read("native/quanttrace/src/hello.c")
    check("0.0.70-slice2bq" in hello, "native version 0.0.70-slice2bq")
    ...
    section("Slice 2bq second nested Mix hop")
    hdr = _read("native/quanttrace/src/quanttrace.h")
    check("mix_nested2_fac" in hdr, "ABI has mix_nested2_fac")
    check("mix_nested2_lightpath_enable" in hdr, "ABI has mix_nested2_lightpath_enable")
    check("mix_nested2_closure1_kind" in hdr, "ABI has mix_nested2_closure1_kind")
    hello = _read("native/quanttrace/src/hello.c")
    check("0.0.70-slice2bq" in hello, "native version 0.0.70-slice2bq")
    sync_src = _read("scenequant/quanttrace/sync.py")
    check("_pack_nested2_mix_shader" in sync_src, "sync has nested2 packer")
    check("Slice 2bq" in sync_src, "sync names Slice 2bq refuse")
    check("ColorRamp Fac on Mix Shader.004" in sync_src, "ColorRamp leftover names Mix.004")
    ...
    check(int(nest["mix_nested2_lightpath_enable"]) == 0, "nested identity includes nested2 enable 0")
    bqscene = _read("tools/_quanttrace_slice2bq_scene.py")
    check("refuse_colorramp" in bqscene, "2bq scene has ColorRamp refuse")
    check("0.35" in bqscene, "2bq CLAIM Mix.004 Fac 0.35")
    bqsmoke = _read("tools/_quanttrace_slice2bq_smoke.py")
    check("pack_scene" in bqsmoke and "render_qt_scene_rgba" in bqsmoke, "2bq smoke packs QT_Scene")
    check("refuse_colorramp" in bqsmoke and "Slice 2bq" in bqsmoke, "2bq smoke expects ColorRamp refuse")
