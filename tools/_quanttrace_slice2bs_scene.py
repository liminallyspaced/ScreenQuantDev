# QuantTrace Slice 2bs: ColorRamp.Fac <- MATH (loft ColorRamp.002).
...
def build_slice2bs_scene(
    image_path="/tmp/qt_slice2bs_env.exr",
    *,
    mode="claim",
):
    ...
    mode:
      ...
      refuse_noise     — ColorRamp.Fac←Noise named REFUSE Slice 2bs
      refuse_add       — Mix.004 ← Add Shader REFUSE
      refuse_third     — third Mix hop named REFUSE Slice 2bs
      refuse_linked    — Glass.Color linked REFUSE Slice 2bs
    """
    ...
    if mode_key not in allowed:
        raise RuntimeError(f"mode={mode!r} refused (Slice 2bs)")
    ...
    if mode_key == "claim":
        ...
        # Glass+Transparent leaves. ColorRamp.Fac <- MATH Backfacing x HueSat.
        lp = nt.nodes.new("ShaderNodeLightPath")
...
def main():
    ...
    build_slice2bs_scene(image_path=args.image, mode=args.mode)
