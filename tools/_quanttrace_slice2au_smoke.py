# pack_scene TEX_ENVIRONMENT×0 MULTIPLY → world Strength → Session vs stock (Slice 2au).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2AU_PAIR", path, "wh", w, h)
...
def _expected_for_mode(mode, strength):
    ...
    if mode == "env_mul0_add20":
        base["world_strength"] = 20.0
        base["world_image_path_empty"] = False
    elif mode == "env_mul0":
        base["world_strength"] = 0.0
        base["world_image_path_empty"] = False
    elif mode in ("math_nest3", "math_mul"):
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2au_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2au_stock.exr")
    ...
    if args.mode == "loft_probe":
        ...
        try:
            ...
            print(
                "QUANTTRACE_SLICE2AU_LOFT packed unexpectedly",
                "world_strength", wi.get("world_strength"),
                "world_image_path", wi.get("world_image_path"),
            )
            return 1
        except qt_sync.QuantTraceSyncError as e:
            print("QUANTTRACE_SLICE2AU_LOFT_REFUSE", type(e).__name__, str(e))
            print("QUANTTRACE_SLICE2AU_SMOKE OK loft_probe")
            return 0

    sys.path.insert(0, os.path.join(root, "tools"))
    import _quanttrace_slice2au_scene as sc2au

    scene, cube_obj, lamp, cam, img = sc2au.build_slice2au_scene(
    ...
    if args.mode in ("math_nest4", "env_mul_nonzero", "env_add"):
        try:
            ...
            msg = str(e)
            print("QUANTTRACE_SLICE2AU_REFUSE", type(e).__name__, msg)
            ...
            elif args.mode == "env_mul_nonzero":
                if "2au" not in msg or "non-zero" not in msg.lower():
                    raise RuntimeError(f"env_mul_nonzero refused but message unexpected: {msg}")
            else:
                if "2au" not in msg or "ADD" not in msg:
                    raise RuntimeError(f"env_add refused but message unexpected: {msg}")
            print("QUANTTRACE_SLICE2AU_SMOKE OK refuse")
            return 0
        raise RuntimeError(
    ...
    if args.render_stock:
        ...
        print(
            "QUANTTRACE_SLICE2AU_STOCK wall", round(time.perf_counter() - t_s, 3),
            "out", args.stock_out,
        )
        if not args.compare:
    ...
    print(
        "QUANTTRACE_SLICE2AU_SMOKE packed",
        "mode", args.mode,
    ...
    if args.mode in ("hdr", "env_mul0_add20", "env_mul0", "math_nest3", "math_mul", "unlinked"):
    ...
    if not _color_close(got_c, expected["world_color"]):
        if args.mode not in (
            ...
            "env_mul0_add20", "env_mul0", "math_nest3", "math_mul", "unlinked",
        ):
            raise RuntimeError(
    ...
    print(
        "QUANTTRACE_SLICE2AU_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.live_graph:
        unlinked_out = "/tmp/quanttrace_slice2au_stock_unlinked.exr"
        scene_b, *_r = sc2au.build_slice2au_scene(
            image_path=args.image,
            mode="unlinked",
            strength=1.0,
            pull_camera=not args.no_pull_camera,
        ...
        print(
            "QUANTTRACE_SLICE2AU_LIVE",
            "dmax", dmax, "mae", mae, "px_ge_1e-3", px,
        )
        if dmax < 1e-2:
            raise RuntimeError(f"live-graph Δmax={dmax} too small — mul0_add20 no-op?")
        print("QUANTTRACE_SLICE2AU_SMOKE OK live-graph")
        return 0
    ...
    if args.compare:
        ...
        if gate != "PASS":
            print("QUANTTRACE_SLICE2AU_SMOKE DONE", gate)
            return 1
    print("QUANTTRACE_SLICE2AU_SMOKE OK")
    return 0
