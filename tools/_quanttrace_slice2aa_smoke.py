# pack_scene HDR Environment Texture world → Session vs stock Cycles (Slice 2aa).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2aa_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2aa_stock.exr")
    ...
    if args.render_stock:
        ...
        print(
            "QUANTTRACE_SLICE2AA_STOCK wall", round(time.perf_counter() - t_s, 3),
            "out", args.stock_out,
        )
        if not args.compare:
    ...
    print(
        "QUANTTRACE_SLICE2AA_SMOKE packed",
        ...
        "black", args.black_world,
    )
    if not args.black_world:
        if not packed.get("world_image_path"):
            raise RuntimeError("packed world_image_path empty — env not packed")
        if int(packed.get("world_projection", -1)) != (
            1 if args.projection == "MIRROR_BALL" else 0
        ):
            raise RuntimeError(
                f"packed world_projection={packed.get('world_projection')} "
                f"expected {'1' if args.projection == 'MIRROR_BALL' else '0'}"
            )
        if abs(float(packed.get("world_strength", 0.0)) - 1.0) > 1e-6:
            raise RuntimeError(f"packed strength={packed.get('world_strength')}")
    else:
        if packed.get("world_image_path"):
            raise RuntimeError("black-world but path non-empty")
    ...
    print(
        "QUANTTRACE_SLICE2AA_CTYPES path", desc.world_image_path,
    ...
    # Detect constant: all equal within float noise.
    constant = (rgb_max - rgb_min) < 1e-12
    print(
        "QUANTTRACE_SLICE2AA_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
    ...
    if not args.black_world:
        ...
        if constant:
            raise RuntimeError("session Combined constant — env/projection likely dead")
    if args.compare:
        # Visibility: stock must not be all-zero / constant for HDR path.
        if not args.black_world and os.path.isfile(args.compare):
            try:
                ...
                print(
                    "QUANTTRACE_SLICE2AA_STOCK_VIS min", smin, "max", smax,
                ...
                print("QUANTTRACE_SLICE2AA_STOCK_VIS skip (no OIIO in this process)")
        ...
        print("QUANTTRACE_SLICE2AA_SMOKE compare rc", rcode)
        if rcode != 0:
            raise SystemExit(rcode)
    print("QUANTTRACE_SLICE2AA_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2AA_SMOKE FAIL", type(e).__name__, e)
        raise
