# pack_scene Coat Normal Map TEX_IMAGE cube → Session vs stock Cycles (Slice 2t).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_coatnormal_session.exr")
    ...
    import _quanttrace_coatnormal_scene as nsc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, img = nsc.build_coatnormal_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_COATNORMAL_SMOKE",
        "coat_normal_img", m0.get("coat_normal_image_path"),
        "coat_normal_cs", m0.get("coat_normal_image_colorspace"),
        "strength", m0.get("coat_normal_strength"),
        "vec", m0.get("coat_normal_tex_vector_mode"),
    ...
    print(
        "QUANTTRACE_COATNORMAL_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_COATNORMAL_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_COATNORMAL_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_COATNORMAL_SMOKE FAIL", type(e).__name__, e)
        raise
