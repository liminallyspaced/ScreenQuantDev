# QuantTrace Slice 2ad: locked cube + Normal Map BLENDER_OBJECT / BLENDER_WORLD.
...
def build_slice2ad_scene(
    image_path="/tmp/qt_slice2ad_normal.png",
    *,
    socket="Normal",
    ...
    socket: Normal | CoatNormal | Object | Tangent  (Object/Tangent = 2z/2j reg)
    ...
    if sock == "Object":
        import _quanttrace_slice2z_scene as sc2z
        return sc2z.build_slice2z_scene(
            image_path=image_path, socket="Normal", space="OBJECT",
            strength=strength, base_color=base_color,
        )
    ...
    px = list(img.pixels)
    print(
        "QUANTTRACE_SLICE2AD_IMG", image_path, "size", list(img.size),
    ...
    tex.projection = "FLAT"
    tex.label = "qt_slice2ad_tex"
    nmap = nt.nodes.new("ShaderNodeNormalMap")
    sp = str(space).upper()
    if sp not in ("TANGENT", "OBJECT", "WORLD", "BLENDER_OBJECT", "BLENDER_WORLD"):
        raise RuntimeError(
            f"space={space!r} refused (Slice 2ad: TANGENT/OBJECT/WORLD/"
            "BLENDER_OBJECT/BLENDER_WORLD)"
        )
    nmap.space = sp
    nmap.inputs["Strength"].default_value = float(strength)
    nmap.label = "qt_slice2ad_nmap"
    ...
    print(
        "QUANTTRACE_SLICE2AD_BSDF socket", sock, "space", nmap.space,
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2ad_stock.exr")
    ...
    scene, cube_obj, lamp, cam, img = build_slice2ad_scene(
    ...
    print(
        "QUANTTRACE_SLICE2AD", cube_obj.name,
    ...
    if args.render:
        ...
        print("QUANTTRACE_SLICE2AD wrote", args.out)
    return 0
