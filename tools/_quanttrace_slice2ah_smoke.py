# pack_scene linked world Strength Value → Session vs stock Cycles (Slice 2ah).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2ah_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2ah_stock.exr")
    ...
    import _quanttrace_slice2ah_scene as sc2ah
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()

    strength = float(args.strength)
    scene, cube_obj, lamp, cam, img = sc2ah.build_slice2ah_scene(
    ...
    if args.render_stock:
        ...
        print(
            "QUANTTRACE_SLICE2AH_STOCK wall", round(time.perf_counter() - t_s, 3),
            "out", args.stock_out,
        )
        if not args.compare:
    ...
    print(
        "QUANTTRACE_SLICE2AH_SMOKE packed",
        "mode", args.mode,
    ...
    if abs(got - strength) > 1e-6:
        raise RuntimeError(
            f"packed world_strength={got} expected {strength} (mode={args.mode})"
        )
    ...
    print(
        "QUANTTRACE_SLICE2AH_CTYPES path", desc.world_image_path,
        "proj", desc.world_projection, "strength", desc.world_strength,
        "ver", ver, "is_tracer", is_tr,
    )
    if abs(float(desc.world_strength) - strength) > 1e-6:
        raise RuntimeError(
            f"ctypes world_strength={desc.world_strength} expected {strength}"
        )
    ...
    print(
        "QUANTTRACE_SLICE2AH_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.compare:
        if os.path.isfile(args.compare):
            try:
                ...
                print(
                    "QUANTTRACE_SLICE2AH_STOCK_VIS min", smin, "max", smax,
                ...
                print("QUANTTRACE_SLICE2AH_STOCK_VIS skip (no OIIO in this process)")
        ...
        print("QUANTTRACE_SLICE2AH_SMOKE compare rc", rcode)
        if rcode != 0:
            raise SystemExit(rcode)
    print("QUANTTRACE_SLICE2AH_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2AH_SMOKE FAIL", type(e).__name__, e)
        raise
