# pack_scene Specular Tint / Thin Film / Subsurface TEX_IMAGE cube → Session vs stock (Slice 2u).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2u_session.exr")
    ...
    import _quanttrace_slice2u_scene as sc2u
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, gray, tint, thick = sc2u.build_slice2u_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_SLICE2U_SMOKE socket", args.socket,
        "spec_tint", m0.get("spec_tint_image_path"),
        "film_thick", m0.get("film_thick_image_path"),
        "film_ior", m0.get("film_ior_image_path"),
        "sss_weight", m0.get("sss_weight_image_path"),
        "sss_radius", m0.get("sss_radius_image_path"),
        "sss_scale", m0.get("sss_scale_image_path"),
    ...
    print(
        "QUANTTRACE_SLICE2U_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2U_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_SLICE2U_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2U_SMOKE FAIL", type(e).__name__, e)
        raise
