# pack_scene TEX_IMAGE cube → Session vs stock Cycles (Slice 2f).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_tex_session.exr")
    ...
    scene, cube_obj, lamp, cam, img = texsc.build_tex_scene(image_path=args.image)
    scene.render.resolution_x = scene.render.resolution_y = args.res
    scene.cycles.samples = args.samples
    deps = bpy.context.evaluated_depsgraph_get()
    packed = qt_sync.pack_scene(scene, depsgraph=deps)
    m0 = packed["meshes"][0]
    print("QUANTTRACE_TEX_SMOKE image", m0.get("image_path"),
    ...
    print("QUANTTRACE_TEX_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
          "ver", ver)
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_TEX_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_TEX_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_TEX_SMOKE FAIL", type(e).__name__, e)
        raise
