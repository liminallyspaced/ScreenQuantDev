# pack_scene Emission Color TEX_IMAGE cube → Session vs stock Cycles (Slice 2r).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_emitcolor_session.exr")
    ...
    import _quanttrace_emitcolor_scene as ecsc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, img, str_img = ecsc.build_emitcolor_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_EMITCOLOR_SMOKE socket", args.socket,
        "emit_color_img", m0.get("emit_color_image_path"),
        "emit_color_cs", m0.get("emit_color_image_colorspace"),
        "emit_str_img", m0.get("emit_str_image_path"),
        "emit_str_cs", m0.get("emit_str_image_colorspace"),
    ...
    print(
        "QUANTTRACE_EMITCOLOR_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_EMITCOLOR_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_EMITCOLOR_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_EMITCOLOR_SMOKE FAIL", type(e).__name__, e)
        raise
