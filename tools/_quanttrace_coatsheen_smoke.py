# pack_scene Coat/Sheen/Emission Strength TEX_IMAGE cube → Session vs stock Cycles (Slice 2q).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_coatsheen_session.exr")
    ...
    import _quanttrace_coatsheen_scene as cssc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, img = cssc.build_coatsheen_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_COATSHEEN_SMOKE socket", args.socket,
        "coat_img", m0.get("coat_image_path"),
        "coat_cs", m0.get("coat_image_colorspace"),
        "sheen_img", m0.get("sheen_image_path"),
        "sheen_cs", m0.get("sheen_image_colorspace"),
        "emit_img", m0.get("emit_str_image_path"),
        "emit_cs", m0.get("emit_str_image_colorspace"),
    ...
    print(
        "QUANTTRACE_COATSHEEN_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_COATSHEEN_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_COATSHEEN_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_COATSHEEN_SMOKE FAIL", type(e).__name__, e)
        raise
