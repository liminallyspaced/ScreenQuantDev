# QuantTrace Slice 2m: locked cube + TEX_COORD Camera [→ Mapping] → TEX_IMAGE.
...
def build_camera_scene(
    image_path="/tmp/qt_checker_cam.png",
    *,
    use_mapping=False,
    scale=(2.0, 2.0, 2.0),
    ...
    """TEX_COORD Camera [→ Mapping VECTOR] → TEX_IMAGE Vector → Base Color."""
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_cam_stock.exr")
    ...
    scene, cube_obj, lamp, cam, img = build_camera_scene(
    ...
    print(
        "QUANTTRACE_CAM", cube_obj.name, "mode", args.mode,
    ...
    if args.render:
        ...
        print("QUANTTRACE_CAM wrote", args.out)
    return 0
