# QuantTrace Slice 2bj census: loft SEPARATE_COLOR → Principled.Roughness.
...
for obj in bpy.data.objects:
    ...
    for slot in obj.material_slots:
        ...
        for n in mat.node_tree.nodes:
            ...
            if rough is None or not rough.is_linked:
                continue
            links = list(rough.links)
            if len(links) != 1:
                continue
            ...
            examples.append(
                (obj.name, mat.name, getattr(fs, "name", None),
                 getattr(fn, "mode", None), getattr(cn, "type", None) if cin and cin.is_linked else None)
            )

print("SEPARATE→Roughness count", sum(channels.values()))
