# pack_scene Normal Map Color ← Combine+InvertG vs stock (Slice 2bi).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BI_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BI_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BI_SMOKE loft open", loft_path)
    ...
    try:
        ...
        inv_n = sum(
            1 for m in packed["meshes"] if int(m.get("normal_invert_g_enable") or 0) != 0
        )
        print(
            "PACK_OK",
            "n_meshes", len(packed["meshes"]),
            "n_lights", len(packed["lights"]),
            "invert_g_meshes", inv_n,
            "wall_s", round(wall, 3),
            "version", qt_engine.native_version(),
        )
        print("QUANTTRACE_SLICE2BI_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BI_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2bi_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2bi_stock.exr")
    ...
    import _quanttrace_slice2bi_scene as sc2bi
    scene, cube_obj, lamp, cam, img = sc2bi.build_slice2bi_scene(
    ...
    refuse = args.expect_refuse or args.mode in ("fac_linked", "invert_r", "hsv")
    deps = bpy.context.evaluated_depsgraph_get()
    try:
        packed = qt_sync.pack_scene(scene, depsgraph=deps)
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("QUANTTRACE_SLICE2BI_PACK_REFUSE", msg)
        if refuse:
            if "Slice 2bi" in msg:
                print("QUANTTRACE_SLICE2BI_SMOKE REFUSE_OK", msg)
                return 0
            raise RuntimeError(f"refuse missing expected tag Slice 2bi: {msg}")
        raise

    if refuse:
        raise RuntimeError("expected Slice 2bi refuse but pack_scene succeeded")

    m0 = packed["meshes"][0]
    got_en = int(
        m0.get("normal_invert_g_enable", 0)
        if m0.get("normal_invert_g_enable") is not None
        else 0
    )
    got_fac = float(
        m0.get("normal_invert_g_fac", 1.0)
        if m0.get("normal_invert_g_fac") is not None
        else 1.0
    )
    got_path = bool(m0.get("normal_image_path") or "")
    print(
        "QUANTTRACE_SLICE2BI_PACKED",
        "mode", args.mode,
        "invert_g_enable", got_en,
        "invert_g_fac", got_fac,
        "normal_img", got_path,
        "version", qt_engine.native_version(),
    )
    if args.mode == "claim":
        if got_en != 1:
            raise RuntimeError(f"claim invert_g_enable={got_en} expected 1")
        if abs(got_fac - 1.0) > 1e-6:
            raise RuntimeError(f"claim invert_g_fac={got_fac} expected 1.0")
        if not got_path:
            raise RuntimeError("claim expected normal_image_path")
    elif args.mode in ("bypass", "normal"):
        if got_en != 0:
            raise RuntimeError(f"{args.mode} invert_g_enable={got_en} expected 0 (2j identity)")
        if not got_path:
            raise RuntimeError(f"{args.mode} expected normal_image_path")
    elif args.mode == "mix":
        if got_en != 0:
            raise RuntimeError(f"mix identity invert_g_enable={got_en} expected 0")
    elif args.mode == "curves":
        if got_en != 0:
            raise RuntimeError(f"curves identity invert_g_enable={got_en} expected 0")
    ...
    if args.live_graph:
        ...
        scene_z, *_ = sc2bi.build_slice2bi_scene(
        ...
        scene_z.render.filepath = "/tmp/qt_slice2bi_live_bypass.exr"
        ...
        rcode = subprocess.call([
            ...
            args.stock_out, "/tmp/qt_slice2bi_live_bypass.exr",
        ])
        print("QUANTTRACE_SLICE2BI_LIVE compare rc", rcode)
        return 0
    ...
    print(
        "QUANTTRACE_SLICE2BI_CTYPES",
        "invert_g_enable", int(desc.meshes[0].normal_invert_g_enable),
        "invert_g_fac", float(desc.meshes[0].normal_invert_g_fac),
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BI_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        if os.path.isfile(args.stock_out):
            os.unlink(args.stock_out)
        bpy.ops.render.render(write_still=True)
        print("QUANTTRACE_SLICE2BI_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2BI_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [x.strip() for x in pair.split(",") if x.strip()] or [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-invert-g-32-pair.png")
            ws = "/workspace/quanttrace-invert-g-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BI_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
            raise SystemExit(rcode)

    print("QUANTTRACE_SLICE2BI_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2BI_SMOKE FAIL", type(e).__name__, e)
        raise
