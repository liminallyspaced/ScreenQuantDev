"""Slice 2bu census: Mix.002 / Mix.003 under Add in loft Realistic_Glass_01 after 2bt.
...
Cite Cycles shader_nodes.h / shader_nodes.cpp:
  ...
  GlassBsdfNode / TransparentBsdfNode / GlossyBsdfNode / SubsurfaceScatteringNode /
  TranslucentBsdfNode / RefractionBsdfNode
  RGBRampNode: Fac in, Color/Alpha out (ramp LUT)

Do NOT invent the graph — dump Mix.002 and Mix.003 FULLY (Fac, Shader, Shader_001,
peel REROUTE, linked vs unlinked Color/Roughness/IOR, nested Mix.001, Refraction,
SSS ColorRamp). Print first PACK_FAIL after trial pack of Realistic_Glass_01.
"""
...
print("\n=== Slice 2bt ColorRamp.002 Fac MATH dump ===")
...
print("\n=== Slice 2bt ColorRamp.002 Fac graph (MATH / NEW_GEOMETRY / HueSat) ===")
...
print("\n=== Slice 2bu dry pack probe (first PACK_FAIL after 2bt) ===")
try:
    import os, sys
...
print("\n=== Slice 2bu Mix.002 / Mix.003 FULL dump (Mix-under-Add) ===")
print("Cite AddClosureNode Closure1/Closure2; GlossyBsdfNode Color/Roughness/Distribution;")
print("SubsurfaceScatteringNode Color/Scale/Radius/IOR/Roughness; TranslucentBsdfNode Color;")
print("MixClosureNode Fac/Closure1/Closure2")
mat = bpy.data.materials.get("Realistic_Glass_01")
if mat is None:
    print("Realistic_Glass_01 NOT FOUND")
else:
    for name in ("Mix Shader.002", "Mix Shader.003", "Add Shader", "Glossy BSDF",
                 "Glossy BSDF.001", "Subsurface Scattering", "Translucent BSDF",
                 "Refraction BSDF", "Glass BSDF"):
        n = mat.node_tree.nodes.get(name)
        print(f"\n--- by name {name!r} found={n is not None} ---")
        if n is None:
            continue
        ...
        dump_node_inputs(n, "  ")
        if t == "MIX_SHADER":
            dump_mix_full(n, name, "  ")
        if t == "ADD_SHADER":
            dump_add_full(n, "  ")
        if t == "BSDF_GLOSSY":
            print("  dist", getattr(n, "distribution", None))
        if t == "SUBSURFACE_SCATTERING":
            print("  falloff", getattr(n, "falloff", None), "subsurface_method", getattr(n, "subsurface_method", None))
    print("\n--- all Glossy / SSS / Translucent / Refraction / Add in Realistic_Glass_01 ---")
    for n in mat.node_tree.nodes:
        t = getattr(n, "type", None)
        if t in ("BSDF_GLOSSY", "SUBSURFACE_SCATTERING", "BSDF_TRANSLUCENT",
                 "BSDF_REFRACTION", "ADD_SHADER"):
            print(f"  node={n.name} type={t}")
            dump_node_inputs(n, "    ")
            if t == "BSDF_GLOSSY":
                print("    dist", getattr(n, "distribution", None))
            if t == "SUBSURFACE_SCATTERING":
                print("    falloff", getattr(n, "falloff", None))
            if t == "ADD_SHADER":
                dump_add_full(n, "    ")
