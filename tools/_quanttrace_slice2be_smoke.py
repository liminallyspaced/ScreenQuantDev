# pack_scene Invert -> Principled.Roughness -> Session vs stock (Slice 2be).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BE_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BE_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BE_SMOKE loft open", loft_path)
    ...
    try:
        ...
        inv_n = sum(
            1 for m in packed["meshes"] if int(m.get("rough_invert_enable") or 0)
        )
        print(
            "PACK_OK",
        ...
        print("QUANTTRACE_SLICE2BE_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BE_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2be_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2be_stock.exr")
    ...
    import _quanttrace_slice2be_scene as sc2be
    scene, cube_obj, lamp, cam, img = sc2be.build_slice2be_scene(
    ...
    try:
        ...
        msg = str(e)
        print("QUANTTRACE_SLICE2BE_PACK_REFUSE", msg)
        if refuse:
            if "Slice 2be" in msg:
                print("QUANTTRACE_SLICE2BE_SMOKE REFUSE_OK", msg)
                return 0
            raise RuntimeError(f"refuse missing Slice 2be tag: {msg}")
        raise

    if refuse:
        raise RuntimeError("expected Slice 2be refuse but pack_scene succeeded")

    m0 = packed["meshes"][0]
    got_en = int(m0.get("rough_invert_enable", 0) or 0)
    got_fac = float(
        m0.get("rough_invert_fac", 1.0) if m0.get("rough_invert_fac") is not None else 1.0
    )
    got_ramp = int(m0.get("rough_ramp_n", 0) or 0)
    got_img = bool(m0.get("rough_image_path") or "")
    print(
        "QUANTTRACE_SLICE2BE_PACKED",
        "mode", args.mode,
        "rough_invert_enable", got_en,
        "rough_invert_fac", got_fac,
        "rough_ramp_n", got_ramp,
        "rough_image", got_img,
        "roughness", m0.get("roughness"),
        "version", qt_engine.native_version(),
    )
    if args.mode in ("invert", "invert_full"):
        if got_en != 1:
            raise RuntimeError(f"rough_invert_enable={got_en} expected 1")
        if not got_img:
            raise RuntimeError("CLAIM invert expected rough_image_path")
        if got_ramp != 0:
            raise RuntimeError(f"CLAIM invert expected ramp_n=0 got {got_ramp}")
        want = 0.08333335816860199 if args.mode == "invert" else 1.0
        if not _fclose(got_fac, want, 1e-6):
            raise RuntimeError(f"rough_invert_fac={got_fac} expected {want}")
    elif args.mode == "invert_ramp":
        if got_en != 1:
            raise RuntimeError(f"invert_ramp enable={got_en} expected 1")
        if got_ramp != 257:
            raise RuntimeError(f"invert_ramp ramp_n={got_ramp} expected 257")
        if got_img:
            raise RuntimeError("invert_ramp unexpected rough_image_path")
    elif args.mode == "invert_const":
        if got_en != 0:
            raise RuntimeError(f"invert_const should fold enable=0 got {got_en}")
    elif args.mode == "tex":
        if got_en != 0:
            raise RuntimeError(f"tex 2i identity enable={got_en} expected 0")
        if not got_img:
            raise RuntimeError("tex 2i expected rough_image_path")
    elif args.mode == "ramp":
        if got_en != 0:
            raise RuntimeError(f"ramp 2ba identity enable={got_en} expected 0")
        if got_ramp == 0:
    ...
    if args.live_graph:
        ...
        scene_z, *_ = sc2be.build_slice2be_scene(
        ...
        scene_z.render.filepath = "/tmp/qt_slice2be_live_noinvert.exr"
        ...
        rcode = subprocess.call([
            ...
            args.stock_out, "/tmp/qt_slice2be_live_noinvert.exr",
        ])
        print("QUANTTRACE_SLICE2BE_LIVE compare rc", rcode)
        return 0
    ...
    print(
        "QUANTTRACE_SLICE2BE_CTYPES",
        "invert_enable", int(desc.meshes[0].rough_invert_enable),
        "fac", float(desc.meshes[0].rough_invert_fac),
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BE_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        if os.path.isfile(args.stock_out):
            os.unlink(args.stock_out)
        bpy.ops.render.render(write_still=True)
        print("QUANTTRACE_SLICE2BE_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2BE_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [x.strip() for x in pair.split(",") if x.strip()] or [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-invert-rough-32-pair.png")
            ws = "/workspace/quanttrace-invert-rough-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BE_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
            raise SystemExit(rcode)

    print("QUANTTRACE_SLICE2BE_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2BE_SMOKE FAIL", type(e).__name__, e)
        raise
