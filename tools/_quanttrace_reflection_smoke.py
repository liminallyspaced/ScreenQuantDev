# pack_scene TEX_COORD Reflection cube → Session vs stock Cycles (Slice 2n).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_refl_session.exr")
    ...
    import _quanttrace_reflection_scene as rsc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, img = rsc.build_reflection_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_REFL_SMOKE image", m0.get("image_path"),
        "mode", args.mode,
    ...
    print(
        "QUANTTRACE_REFL_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_REFL_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_REFL_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_REFL_SMOKE FAIL", type(e).__name__, e)
        raise
