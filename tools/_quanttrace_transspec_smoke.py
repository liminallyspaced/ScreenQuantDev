# pack_scene Transmission/Specular TEX_IMAGE cube → Session vs stock Cycles (Slice 2p).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_transspec_session.exr")
    ...
    import _quanttrace_transspec_scene as tssc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, img = tssc.build_transspec_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_TRANSSPEC_SMOKE socket", args.socket,
        "trans_img", m0.get("trans_image_path"),
        "trans_cs", m0.get("trans_image_colorspace"),
        "spec_img", m0.get("spec_image_path"),
        "spec_cs", m0.get("spec_image_colorspace"),
    ...
    print(
        "QUANTTRACE_TRANSSPEC_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_TRANSSPEC_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_TRANSSPEC_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_TRANSSPEC_SMOKE FAIL", type(e).__name__, e)
        raise
