# pack_scene TEX_COORD Camera cube → Session vs stock Cycles (Slice 2m).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_cam_session.exr")
    p.add_argument("--compare", default="")
    p.add_argument("--image", default="/tmp/qt_checker_cam.png")
    ...
    import _quanttrace_camera_scene as csc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, img = csc.build_camera_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_CAM_SMOKE image", m0.get("image_path"),
        "mode", args.mode,
    ...
    print(
        "QUANTTRACE_CAM_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_CAM_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_CAM_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_CAM_SMOKE FAIL", type(e).__name__, e)
        raise
