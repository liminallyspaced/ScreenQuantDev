# pack_scene Noise -> ColorRamp.Fac -> Principled.Roughness -> Session vs stock (Slice 2bb).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BB_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BB_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BB_SMOKE loft open", loft_path)
    ...
    try:
        ...
        noise_n = sum(
            1 for m in packed["meshes"] if int(m.get("rough_ramp_noise_enable") or 0)
        )
        print(
            "PACK_OK",
            ...
            "ramp_meshes", ramp_n,
            "noise_ramp_meshes", noise_n,
            "wall_s", round(wall, 3),
            "version", qt_engine.native_version(),
        )
        print("QUANTTRACE_SLICE2BB_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BB_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2bb_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2bb_stock.exr")
    ...
    scene, cube_obj, lamp, cam, img = sc2bb.build_slice2bb_scene(
    ...
    refuse_modes = ("fresnel", "mix", "vector_linked")
    if args.mode in refuse_modes:
        try:
            qt_sync.pack_scene(scene, depsgraph=bpy.context.evaluated_depsgraph_get())
            print("QUANTTRACE_SLICE2BB_SMOKE", args.mode, "unexpectedly packed")
            return 1
        except qt_sync.QuantTraceSyncError as e:
            print("REFUSE", str(e))
            print("QUANTTRACE_SLICE2BB_SMOKE", args.mode, "REFUSE Slice 2bb")
            return 0
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_SLICE2BB_PACKED",
        "rough_ramp_n", m0.get("rough_ramp_n"),
        "rough_ramp_interpolate", m0.get("rough_ramp_interpolate"),
        "rough_ramp_fac", m0.get("rough_ramp_fac"),
        "noise_enable", m0.get("rough_ramp_noise_enable"),
        "noise_dims", m0.get("rough_ramp_noise_dimensions"),
        "noise_type", m0.get("rough_ramp_noise_type"),
        "noise_scale", m0.get("rough_ramp_noise_scale"),
        "noise_detail", m0.get("rough_ramp_noise_detail"),
        "noise_distortion", m0.get("rough_ramp_noise_distortion"),
        "noise_use_color", m0.get("rough_ramp_noise_use_color"),
        "rough_image", bool(m0.get("rough_image_path")),
        "version", qt_engine.native_version(),
    )
    mode_key = args.mode
    if mode_key in ("noise", "noise_color"):
        assert int(m0.get("rough_ramp_n") or 0) > 0
        assert int(m0.get("rough_ramp_noise_enable") or 0) == 1
        assert not (m0.get("rough_image_path") or "")
        assert abs(float(m0.get("rough_ramp_noise_scale") or 0) - 150.0) < 1e-4
        assert abs(float(m0.get("rough_ramp_noise_detail") or 0) - 16.0) < 1e-4
        assert abs(float(m0.get("rough_ramp_noise_distortion") or 0) - 0.2) < 1e-4
        if mode_key == "noise":
            assert int(m0.get("rough_ramp_noise_use_color") or 0) == 0
        else:
            assert int(m0.get("rough_ramp_noise_use_color") or 0) == 1
    elif mode_key == "ramp":
        assert int(m0.get("rough_ramp_n") or 0) > 0
        assert int(m0.get("rough_ramp_noise_enable") or 0) == 0
    ...
    if args.live_graph:
        ...
        scene_z, *_ = sc2bb.build_slice2bb_scene(
            image_path=args.image, mode="fac_unlinked"
        )
        scene_z.render.resolution_x = scene_z.render.resolution_y = args.res
        scene_z.cycles.samples = args.samples
        scene_z.render.filepath = "/tmp/qt_slice2bb_live_nonoise.exr"
        ...
        rcode = subprocess.call([
            ...
            args.stock_out, "/tmp/qt_slice2bb_live_nonoise.exr",
        ])
        print("QUANTTRACE_SLICE2BB_LIVE compare rc", rcode)
        return 0
    ...
    print(
        "QUANTTRACE_SLICE2BB_CTYPES",
        "ramp_n", int(desc.meshes[0].rough_ramp_n),
        "noise_en", int(desc.meshes[0].rough_ramp_noise_enable),
        "scale", float(desc.meshes[0].rough_ramp_noise_scale),
        "detail", float(desc.meshes[0].rough_ramp_noise_detail),
        "dist", float(desc.meshes[0].rough_ramp_noise_distortion),
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BB_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        if os.path.isfile(args.stock_out):
            os.unlink(args.stock_out)
        bpy.ops.render.render(write_still=True)
        print("QUANTTRACE_SLICE2BB_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if cmp_path and os.path.isfile(cmp_path):
        ...
        print("QUANTTRACE_SLICE2BB_SMOKE compare rc", rcode)
        if args.pair_png:
            paths = [x.strip() for x in args.pair_png.split(",") if x.strip()]
            proof = os.path.join(
                root, "docs", "proof", "quanttrace-noise-colorramp-rough-32-pair.png"
            )
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BB_PAIR FAIL", type(e).__name__, e)
    return 0
