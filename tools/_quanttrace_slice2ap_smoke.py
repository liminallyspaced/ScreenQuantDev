# pack_scene Bright/Contrast → world Color → Session vs stock Cycles (Slice 2ap).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2AP_PAIR", path, "wh", w, h)
...
def _expected_for_mode(mode, strength):
    ...
    if mode == "rgb_bc":
        ...
        base["world_bright"] = 0.08
        base["world_contrast"] = 0.05
    elif mode == "rgb_gamma":
        base["world_color"] = (1.0, 0.25, 0.1)
        base["world_gamma"] = 2.2
    elif mode == "rgb":
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2ap_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2ap_stock.exr")
    p.add_argument("--compare", default="")
    p.add_argument("--render-stock", action="store_true", default=False)
    p.add_argument("--image", default="/tmp/qt_slice2ap_env.exr")
    p.add_argument(
        "--mode",
        choices=(
            "rgb_bc", "rgb_gamma_hsv_bc", "hdr_bc",
            "rgb_gamma", "rgb", "hdr", "nishita", "teximage", "noise", "unlinked_rgb",
        ),
        default="rgb_bc",
    )
    ...
    scene, cube_obj, lamp, cam, img = sc2ap.build_slice2ap_scene(
    ...
    if args.mode == "noise":
        try:
            ...
            msg = str(e)
            print("QUANTTRACE_SLICE2AP_REFUSE", type(e).__name__, msg)
            if "Slice 2ap" not in msg and "Noise" not in msg and "TEX_NOISE" not in msg and "noise" not in msg.lower():
                if "refused" not in msg.lower():
                    raise RuntimeError(f"noise refused but message unexpected: {msg}")
            print("QUANTTRACE_SLICE2AP_SMOKE OK refuse")
            return 0
        raise RuntimeError(
    ...
    if args.render_stock:
        ...
        print(
            "QUANTTRACE_SLICE2AP_STOCK wall", round(time.perf_counter() - t_s, 3),
            "out", args.stock_out,
        )
        if not args.compare:
    ...
    print(
        "QUANTTRACE_SLICE2AP_SMOKE packed",
        "mode", args.mode,
        ...
        "expected_bc", (expected["world_bright"], expected["world_contrast"]),
        "expected_sky", expected["world_sky_type"],
    )
    if args.mode in ("hdr", "hdr_bc"):
    ...
    print(
        "QUANTTRACE_SLICE2AP_CTYPES env_path", desc.world_image_path,
    ...
    if not _fclose(desc.world_bright, expected["world_bright"]):
        raise RuntimeError(
            f"ctypes world_bright={desc.world_bright} expected {expected['world_bright']}"
        )
    if not _fclose(desc.world_contrast, expected["world_contrast"]):
        raise RuntimeError(
            f"ctypes world_contrast={desc.world_contrast} expected {expected['world_contrast']}"
        )
    ...
    print(
        "QUANTTRACE_SLICE2AP_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.compare:
        if os.path.isfile(args.compare):
            try:
                ...
                print(
                    "QUANTTRACE_SLICE2AP_STOCK_VIS min", smin, "max", smax,
                    "constant", smax - smin < 1e-12,
                )
                if smax == 0.0:
                    raise RuntimeError("stock Combined all-zero — world not visible")
            except ImportError:
                print("QUANTTRACE_SLICE2AP_STOCK_VIS skip (no OIIO in this process)")
        ...
        print("QUANTTRACE_SLICE2AP_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-bright-contrast-32-pair.png")
            ws = "/workspace/quanttrace-bright-contrast-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2AP_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
    ...
    if args.live_graph:
        if args.mode != "rgb_bc":
            raise RuntimeError("--live-graph requires --mode rgb_bc")
        unlinked_out = "/tmp/quanttrace_slice2ap_stock_unlinked_rgb.exr"
        scene_b, *_rest = sc2ap.build_slice2ap_scene(
            image_path=args.image,
            mode="unlinked_rgb",
            strength=args.strength,
            pull_camera=not args.no_pull_camera,
        ...
        bpy.ops.render.render(write_still=True)
        print(
            "QUANTTRACE_SLICE2AP_STOCK_UNLINKED wall",
        ...
        print(
            "QUANTTRACE_SLICE2AP_LIVE rgb_bc_vs_unlinked_rgb",
            "dmax", dmax, "mae", mae, "px>=1e-3", n_gt,
            "of", da.shape[0] * da.shape[1],
        )
        if dmax <= 1e-3:
            raise RuntimeError(
                f"live-graph rgb_bc vs unlinked RGB Δmax={dmax} not > 1e-3 "
                "(bright/contrast not visible — camera/GI)"
            )

    print("QUANTTRACE_SLICE2AP_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2AP_SMOKE FAIL", type(e).__name__, e)
        raise
