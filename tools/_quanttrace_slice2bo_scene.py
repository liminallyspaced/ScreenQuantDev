# QuantTrace Slice 2bo: Mix Shader Fac←MATH Light Path nest + Glass+Transparent.
...
def build_slice2bo_scene(
    image_path="/tmp/qt_slice2bo_env.exr",
    *,
    mode="claim",
):
    """Locked cube + backplate; Mix Fac MATH claim + 2bn identity.

    mode:
      claim            — loft Fac MATH: MAXIMUM(MAXIMUM(Is Shadow, Is Reflection),
                         GREATER_THAN(Ray Depth, 6)) + Glass+Transparent
      unlinked_fac     — Mix Fac=0.85 unlinked (2bn identity, math enable=0)
      lightpath_shadow — Fac←Light Path Is Shadow Ray (2bn identity)
      glass_only       — pure Glass identity skip (2bm)
      mix / invert / bump_sep / hdr — prior regressions
      refuse_nested    — Mix(Glass, Mix(...)) named REFUSE Slice 2bo
      refuse_tex       — Mix Fac←TEX named REFUSE
      refuse_linked    — Glass.Color linked named REFUSE
      refuse_add       — Mix Glass+Add named REFUSE
      live_bypass_hint — same as claim (smoke compares vs 2bn-bypass)
    """
    ...
    if mode_key not in allowed:
        raise RuntimeError(f"mode={mode!r} refused (Slice 2bo)")
    ...
    if mode_key in ("claim", "live_bypass_hint"):
        # Loft Realistic_Glass_01 Fac MATH (census 2026-08-30 7am):
        # MAXIMUM(MAXIMUM(Is Shadow Ray, Is Reflection Ray),
        #         GREATER_THAN(Ray Depth, 6.0))
        lp = nt.nodes.new("ShaderNodeLightPath")
        ...
        _link_mix(_mk_glass(), _mk_trans(), fac_from=rootm.outputs["Value"])
        bpy.context.view_layer.update()
        return scene, cube_obj, lamp, cam, img
    ...
    if mode_key == "refuse_tex":
        glass = _mk_glass()
        trans = _mk_trans()
        tex = nt.nodes.new("ShaderNodeTexImage")
        _link_mix(glass, trans, fac_from=tex.outputs.get("Color") or tex.outputs[0])
        bpy.context.view_layer.update()
        return scene, cube_obj, lamp, cam, img
...
if __name__ == "__main__":
    ...
    ap.add_argument("--image", default="/tmp/qt_slice2bo_env.exr")
    args = ap.parse_args(_argv())
    build_slice2bo_scene(image_path=args.image, mode=args.mode)
    print("SLICE2BO_SCENE_OK", args.mode)
