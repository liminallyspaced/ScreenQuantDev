# QuantTrace Slice 2br: second nested Mix Shader hop (loft Mix Shader.004).
...
def build_slice2br_scene(
    image_path="/tmp/qt_slice2br_env.exr",
    *,
    mode="claim",
):
    ...
    mode:
      claim            — Outer MATH + Mix.005 Is Shadow + Mix.004 Fac←ColorRamp
                         (loft ColorRamp.002 stops) Glass+Transparent leaves
      ...
      refuse_noise     — ColorRamp.Fac←Noise named REFUSE Slice 2br
      refuse_add       — Mix.004 ← Add Shader REFUSE
      refuse_third     — third Mix hop named REFUSE Slice 2br
      refuse_linked    — Glass.Color linked REFUSE Slice 2br
    """
    ...
    if mode_key not in allowed:
        raise RuntimeError(f"mode={mode!r} refused (Slice 2br)")
    ...
    def _loft_colorramp():
        ...
        # Fac UNLINKED (loft Fac<-MATH Backfacing*HueSat is a named leftover).
    ...
    if mode_key == "claim":
        ...
        # Glass+Transparent leaves. ColorRamp.Fac unlinked.
        lp = nt.nodes.new("ShaderNodeLightPath")
...
def main():
    ...
    build_slice2br_scene(image_path=args.image, mode=args.mode)
