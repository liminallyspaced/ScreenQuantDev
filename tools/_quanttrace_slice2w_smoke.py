# pack_scene Anisotropic / Rotation / Tangent TEX_IMAGE cube → Session vs stock (Slice 2w).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2w_session.exr")
    ...
    scene, cube_obj, lamp, cam, gray = sc2w.build_slice2w_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_SLICE2W_SMOKE socket", args.socket,
        "aniso", m0.get("aniso_image_path"),
        "aniso_rot", m0.get("aniso_rot_image_path"),
        "tangent", m0.get("tangent_image_path"),
    ...
    print(
        "QUANTTRACE_SLICE2W_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2W_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_SLICE2W_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2W_SMOKE FAIL", type(e).__name__, e)
        raise
