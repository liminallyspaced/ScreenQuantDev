# pack_scene Mapping/TEX_COORD cube → Session vs stock Cycles (Slice 2h).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_map_session.exr")
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_MAP_SMOKE image", m0.get("image_path"),
        "mode", args.mode,
    ...
    print(
        "QUANTTRACE_MAP_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_MAP_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_MAP_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_MAP_SMOKE FAIL", type(e).__name__, e)
        raise
