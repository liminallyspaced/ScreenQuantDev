# pack_scene Mix Glass+Transparent -> Session vs stock (Slice 2bp).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BP_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BP_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BP_SMOKE loft open", loft_path)
    ...
    try:
        ...
        print("QUANTTRACE_SLICE2BP_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BP_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2bp_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2bp_stock.exr")
    ...
    p.add_argument(
        "--mode",
        choices=(
            "claim", "flat_math", "unlinked_fac", "lightpath_shadow", "glass_only",
    ...
    import _quanttrace_slice2bp_scene as sc2bo
    scene, cube_obj, lamp, cam, img = sc2bo.build_slice2bp_scene(
    ...
    try:
        ...
        msg = str(e)
        print("QUANTTRACE_SLICE2BP_PACK_REFUSE", msg)
        if refuse:
            if "Slice 2bp" in msg:
                print("QUANTTRACE_SLICE2BP_SMOKE REFUSE_OK", msg)
                return 0
            raise RuntimeError(f"refuse missing Slice 2bp tag: {msg}")
        raise

    if refuse:
        raise RuntimeError("expected Slice 2bp refuse but pack_scene succeeded")
    ...
    if args.mode == "claim":
        ...
        if int(m0.get("mix_shader_math_op", -1)) != 13:
            raise RuntimeError(f"claim root op not MAXIMUM: {m0.get('mix_shader_math_op')}")
        if int(m0.get("mix_shader_math_a_op", -1)) != 13:
            raise RuntimeError(f"claim A op not MAXIMUM: {m0.get('mix_shader_math_a_op')}")
        if int(m0.get("mix_shader_math_b_op", -1)) != 16:
            raise RuntimeError(f"claim B op not GREATER_THAN: {m0.get('mix_shader_math_b_op')}")
        if int(m0.get("mix_shader_math_a_kind", -1)) != 2:
            raise RuntimeError("claim A not nested")
        if int(m0.get("mix_shader_math_b_kind", -1)) != 2:
            raise RuntimeError("claim B not nested")
        if abs(float(m0.get("mix_shader_math_b2_const", -1)) - 6.0) > 1e-6:
            raise RuntimeError(f"claim GT const not 6: {m0.get('mix_shader_math_b2_const')}")
        # Nested Mix on Closure1 (kind=2), Transparent on Closure2 (kind=1)
        ...
        if nk != {0, 1}:
            raise RuntimeError(f"claim nested leaves not Glass+Transparent: {nk}")
    elif args.mode == "flat_math":
        if got_mix != 1 or got_math != 1 or got_lp != 0:
            raise RuntimeError(f"flat_math mix={got_mix} math={got_math} lp={got_lp}")
        if int(m0.get("mix_closure1_kind", -1)) == 2 or int(m0.get("mix_closure2_kind", -1)) == 2:
            raise RuntimeError("flat_math must keep nested kinds off (2bo identity)")
        if int(m0.get("mix_nested_lightpath_enable", 0) or 0) != 0:
            raise RuntimeError("flat_math nested lp must stay 0")
    elif args.mode == "unlinked_fac":
        if got_mix != 1 or got_math != 0 or got_lp != 0:
            raise RuntimeError(f"unlinked mix={got_mix} math={got_math} lp={got_lp}")
    elif args.mode == "lightpath_shadow":
        if got_mix != 1 or got_lp != 1 or got_math != 0:
            raise RuntimeError(f"lp mix={got_mix} lp={got_lp} math={got_math}")
    ...
    if args.force_pure_glass:
        for m in packed["meshes"]:
            # 2bn-bypass: strip MATH/LightPath to unlinked Fac (or pure Glass).
            m["mix_shader_math_enable"] = 0
            m["mix_shader_lightpath_enable"] = 0
            if int(m.get("mix_shader_enable") or 0):
                # keep Mix Glass+Transparent with unlinked Fac
                pass
        print("QUANTTRACE_SLICE2BP_FORCE_BN_BYPASS")
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BP_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        print("QUANTTRACE_SLICE2BP_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2BP_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [x.strip() for x in pair.split(",") if x.strip()] or [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-nested-mix-32-pair.png")
            ws = "/workspace/quanttrace-nested-mix-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BP_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
            raise SystemExit(rcode)

    print("QUANTTRACE_SLICE2BP_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2BP_SMOKE FAIL", type(e).__name__, e)
        raise
