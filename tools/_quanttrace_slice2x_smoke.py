# pack_scene Bump TEX_IMAGE Height cube → Session vs stock Cycles (Slice 2x).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2x_session.exr")
    ...
    scene, cube_obj, lamp, cam, img = sc2x.build_slice2x_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_SLICE2X_SMOKE socket", args.socket,
        "bump", m0.get("bump_image_path"),
        "bump_cs", m0.get("bump_image_colorspace"),
        "bump_str", m0.get("bump_strength"),
        "bump_dist", m0.get("bump_distance"),
        "bump_inv", m0.get("bump_invert"),
        "bump_vec", m0.get("bump_tex_vector_mode"),
        "normal", m0.get("normal_image_path"),
        "aniso", m0.get("aniso_image_path"),
    ...
    rgb = [buf[i] for i in range(0, n, 4) for _ in (0,)]
    # RGB only
    ...
    print(
        "QUANTTRACE_SLICE2X_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
    ...
    if rgb_max == 0.0:
        raise RuntimeError("session Combined all-zero — bump graph likely missing")
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2X_SMOKE compare rc", rcode)
        if rcode != 0:
            raise SystemExit(rcode)
    print("QUANTTRACE_SLICE2X_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2X_SMOKE FAIL", type(e).__name__, e)
        raise
