# pack_scene Math → world Strength → Session vs stock Cycles (Slice 2ai).
...
def _expected_strength(mode: str, strength: float, mul_a, mul_b, add_a, add_b) -> float:
    if mode == "math_mul":
        return float(mul_a) * float(mul_b)
    if mode == "math_add":
        return float(add_a) + float(add_b)
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2ai_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2ai_stock.exr")
    ...
    expected = _expected_strength(
        args.mode, args.strength, args.mul_a, args.mul_b, args.add_a, args.add_b
    )
    ...
    if args.render_stock:
        ...
        print(
            "QUANTTRACE_SLICE2AI_STOCK wall", round(time.perf_counter() - t_s, 3),
            "out", args.stock_out,
        )
        if not args.compare:
    ...
    print(
        "QUANTTRACE_SLICE2AI_SMOKE packed",
        "mode", args.mode,
    ...
    print(
        "QUANTTRACE_SLICE2AI_CTYPES path", desc.world_image_path,
    ...
    print(
        "QUANTTRACE_SLICE2AI_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.compare:
        if os.path.isfile(args.compare):
            try:
                ...
                print(
                    "QUANTTRACE_SLICE2AI_STOCK_VIS min", smin, "max", smax,
                ...
                print("QUANTTRACE_SLICE2AI_STOCK_VIS skip (no OIIO in this process)")
        ...
        print("QUANTTRACE_SLICE2AI_SMOKE compare rc", rcode)
        if rcode != 0:
            raise SystemExit(rcode)
    print("QUANTTRACE_SLICE2AI_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2AI_SMOKE FAIL", type(e).__name__, e)
        raise
