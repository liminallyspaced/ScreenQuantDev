# QuantTrace Slice 2ap: Bright/Contrast → world Background Color.
...
def _add_rgb(wnt, bg, rgb):
    ...
    rgb_node.location = (bg.location[0] - 600, bg.location[1] + 40)
...
def build_slice2ap_scene(
    image_path="/tmp/qt_slice2ap_env.exr",
    *,
    mode="rgb_bc",
    strength=1.0,
    pull_camera=True,
    env_path="/tmp/qt_slice2ap_env.exr",
):
    """Locked cube + AREA; Bright/Contrast on world Color.

    mode:
      rgb_bc            — RGB → BrightContrast 0.2/0.3 → Background. CLAIM.
      rgb_gamma_hsv_bc  — RGB → Gamma 2.2 → HueSat → BrightContrast → BG
      hdr_bc            — 2aa HDR equirect + BrightContrast 0.08/0.05
      rgb_gamma         — Slice 2ao regression (identity BC)
      rgb / hdr / nishita / teximage — identity BC regressions
    ...
    if mode_key not in allowed:
        raise RuntimeError(f"mode={mode!r} refused (Slice 2ap)")
    ...
    if mode_key == "rgb_gamma":
        return sc2ao.build_slice2ao_scene(
            image_path=env_path,
            mode="rgb_gamma",
            strength=float(strength),
            pull_camera=pull_camera,
    ...
    if mode_key == "hdr_bc":
        ...
        bc = _add_bc(wnt, bg, bright=HDR_BRIGHT, contrast=HDR_CONTRAST)
        wnt.links.new(env.outputs["Color"], bc.inputs["Color"])
        wnt.links.new(bc.outputs["Color"], bg.inputs["Color"])
        bg.inputs["Color"].default_value = (0.0, 0.0, 0.0, 1.0)
        print(
            "QUANTTRACE_SLICE2AP_WORLD mode hdr_bc",
            "bright", float(HDR_BRIGHT), "contrast", float(HDR_CONTRAST),
    ...
    if mode_key == "unlinked_rgb":
        ...
        print(
            "QUANTTRACE_SLICE2AP_WORLD mode unlinked_rgb",
        ...
        print(
            "QUANTTRACE_SLICE2AP_WORLD mode noise",
        ...
        if mode_key == "rgb_bc":
            bc = _add_bc(wnt, bg, bright=BC_BRIGHT, contrast=BC_CONTRAST)
            wnt.links.new(rgb_node.outputs[0], bc.inputs["Color"])
            wnt.links.new(bc.outputs["Color"], color_in)
            print(
                "QUANTTRACE_SLICE2AP_WORLD mode rgb_bc",
                "rgb", rgb, "bright", BC_BRIGHT, "contrast", BC_CONTRAST,
                "sock_default", tuple(float(v) for v in color_in.default_value[:3]),
                "strength", float(str_in.default_value),
            )
        elif mode_key == "rgb_gamma_hsv_bc":
            ...
            wnt.links.new(hsv.outputs["Color"], bc.inputs["Color"])
            wnt.links.new(bc.outputs["Color"], color_in)
            print(
                "QUANTTRACE_SLICE2AP_WORLD mode rgb_gamma_hsv_bc",
                "rgb", rgb, "gamma", float(GAMMA),
                "hue", HSV_HUE, "sat", HSV_SAT, "val", HSV_VAL, "fac", HSV_FAC,
                "bright", BC_BRIGHT, "contrast", BC_CONTRAST,
                "sock_default", tuple(float(v) for v in color_in.default_value[:3]),
                "strength", float(str_in.default_value),
            )
        else:
            raise RuntimeError(f"mode={mode_key!r} unhandled (Slice 2ap)")
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2ap_stock.exr")
    ...
    scene, cube_obj, lamp, cam, img = build_slice2ap_scene(
    ...
    print(
        "QUANTTRACE_SLICE2AP", cube_obj.name, "mode", args.mode,
        "strength", args.strength,
    ...
    if args.render:
        ...
        print("QUANTTRACE_SLICE2AP wrote", args.out)
    return 0
