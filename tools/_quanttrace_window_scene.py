# QuantTrace Slice 2n: locked cube + TEX_COORD Window [→ Mapping] → TEX_IMAGE.
...
def build_window_scene(
    image_path="/tmp/qt_checker_win.png",
    *,
    use_mapping=False,
    scale=(2.0, 2.0, 2.0),
    ...
    """TEX_COORD Window [→ Mapping VECTOR] → TEX_IMAGE Vector → Base Color."""
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_win_stock.exr")
    ...
    scene, cube_obj, lamp, cam, img = build_window_scene(
    ...
    print(
        "QUANTTRACE_WIN", cube_obj.name, "mode", args.mode,
    ...
    if args.render:
        ...
        print("QUANTTRACE_WIN wrote", args.out)
    return 0
