# pack_scene Fresnel Fac -> Mix -> Principled Base Color vs stock (Slice 2bf).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BF_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BF_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BF_SMOKE loft open", loft_path)
    ...
    try:
        ...
        print("QUANTTRACE_SLICE2BF_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BF_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2bf_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2bf_stock.exr")
    ...
    import _quanttrace_slice2bf_scene as sc2bf
    scene, cube_obj, lamp, cam, img = sc2bf.build_slice2bf_scene(
    ...
    refuse = args.expect_refuse or args.mode == "fac_noise"
    deps = bpy.context.evaluated_depsgraph_get()
    try:
        packed = qt_sync.pack_scene(scene, depsgraph=deps)
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("QUANTTRACE_SLICE2BF_PACK_REFUSE", msg)
        if refuse:
            if "Slice 2bf" in msg:
                print("QUANTTRACE_SLICE2BF_SMOKE REFUSE_OK", msg)
                return 0
            raise RuntimeError(f"refuse missing Slice 2bf tag: {msg}")
        raise

    if refuse:
        raise RuntimeError("expected Slice 2bf refuse but pack_scene succeeded")
    ...
    got_inv = int(
        m0.get("rough_invert_enable", 0) if m0.get("rough_invert_enable") is not None else 0
    )
    print(
        "QUANTTRACE_SLICE2BF_PACKED",
        "mode", args.mode,
        "base_mix_type", got_type,
        "fresnel_enable", got_en,
        "fresnel_ior", got_ior,
        "curves_n", got_curves,
        "invert_enable", got_inv,
    ...
    if args.mode == "claim":
        ...
        if not (m0.get("image_path") or ""):
            raise RuntimeError("claim expected image_path")
    elif args.mode == "mix":
        if got_en != 0:
        ...
        if got_inv != 1:
            raise RuntimeError(f"invert 2be enable={got_inv} expected 1")
    ...
    if args.live_graph:
        ...
        scene_z, *_ = sc2bf.build_slice2bf_scene(
            image_path=args.image, mode="mix"
        )
        scene_z.render.resolution_x = scene_z.render.resolution_y = args.res
        scene_z.cycles.samples = args.samples
        scene_z.render.filepath = "/tmp/qt_slice2bf_live_unlinked.exr"
        ...
        rcode = subprocess.call([
            ...
            args.stock_out, "/tmp/qt_slice2bf_live_unlinked.exr",
        ])
        print("QUANTTRACE_SLICE2BF_LIVE compare rc", rcode)
        return 0
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BF_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        if os.path.isfile(args.stock_out):
            os.unlink(args.stock_out)
        bpy.ops.render.render(write_still=True)
        print("QUANTTRACE_SLICE2BF_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2BF_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [x.strip() for x in pair.split(",") if x.strip()] or [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-fresnel-fac-32-pair.png")
            ws = "/workspace/quanttrace-fresnel-fac-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BF_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
            raise SystemExit(rcode)

    print("QUANTTRACE_SLICE2BF_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2BF_SMOKE FAIL", type(e).__name__, e)
        raise
