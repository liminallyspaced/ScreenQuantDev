# pack_scene Mix Glass+Transparent -> Session vs stock (Slice 2bs).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BS_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BS_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BS_SMOKE loft open", loft_path)
    ...
    try:
        ...
        print("QUANTTRACE_SLICE2BS_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BS_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2bs_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2bs_stock.exr")
    p.add_argument("--compare", default="")
    p.add_argument("--render-stock", action="store_true", default=False)
    p.add_argument("--image", default="/tmp/qt_slice2bs_env.exr")
    p.add_argument(
        "--mode",
        choices=(
    ...
    import _quanttrace_slice2bs_scene as sc2bo
    scene, cube_obj, lamp, cam, img = sc2bo.build_slice2bs_scene(
    ...
    try:
        ...
        msg = str(e)
        print("QUANTTRACE_SLICE2BS_PACK_REFUSE", msg)
        if refuse:
            if "Slice 2bs" in msg:
                print("QUANTTRACE_SLICE2BS_SMOKE REFUSE_OK", msg)
                return 0
            raise RuntimeError(f"refuse missing Slice 2bs tag: {msg}")
        raise

    if refuse:
        raise RuntimeError("expected Slice 2bs refuse but pack_scene succeeded")
    ...
    if args.force_pure_glass:
        for m in packed["meshes"]:
            # 2bs-bypass: ColorRamp.Fac MATH off -> unlinked Fac (2br ramp path).
            m["mix_nested2_ramp_math_enable"] = 0
        print("QUANTTRACE_SLICE2BS_FORCE_MATH_BYPASS")
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BS_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        print("QUANTTRACE_SLICE2BS_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2BS_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [x.strip() for x in pair.split(",") if x.strip()] or [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-colorramp-fac-math-32-pair.png")
            ws = "/workspace/quanttrace-colorramp-fac-math-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BS_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
            raise SystemExit(rcode)

    print("QUANTTRACE_SLICE2BS_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2BS_SMOKE FAIL", type(e).__name__, e)
        raise
