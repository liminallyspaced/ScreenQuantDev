# pack_scene Coat/Sheen extra TEX_IMAGE cube → Session vs stock Cycles (Slice 2s).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_coatextra_session.exr")
    ...
    import _quanttrace_coatextra_scene as cesc
    qt_engine._reset_native_probe_for_tests()
    assert qt_engine.kernel_ready()
    scene, cube_obj, lamp, cam, gray, tint = cesc.build_coatextra_scene(
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_COATEXTRA_SMOKE socket", args.socket,
        "coat_rough", m0.get("coat_rough_image_path"),
        "coat_ior", m0.get("coat_ior_image_path"),
        "coat_tint", m0.get("coat_tint_image_path"),
        "sheen_rough", m0.get("sheen_rough_image_path"),
        "sheen_tint", m0.get("sheen_tint_image_path"),
    ...
    print(
        "QUANTTRACE_COATEXTRA_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_COATEXTRA_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_COATEXTRA_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_COATEXTRA_SMOKE FAIL", type(e).__name__, e)
        raise
