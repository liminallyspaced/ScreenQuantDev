# pack_scene Normal Map TEX_IMAGE cube → Session vs stock Cycles (Slice 2j).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_normal_session.exr")
    ...
    m0 = packed["meshes"][0]
    print(
        "QUANTTRACE_NORMAL_SMOKE",
        "normal_img", m0.get("normal_image_path"),
        "normal_cs", m0.get("normal_image_colorspace"),
    ...
    print(
        "QUANTTRACE_NORMAL_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
        "ver", ver,
    )
    if rc != 0:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_NORMAL_SMOKE compare rc", r)
        if r != 0:
            raise SystemExit(r)
    print("QUANTTRACE_NORMAL_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_NORMAL_SMOKE FAIL", type(e).__name__, e)
        raise
