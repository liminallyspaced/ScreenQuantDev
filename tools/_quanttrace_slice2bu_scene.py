# QuantTrace Slice 2bu: nested2 Mix <- Add+Glossy/SSS (loft Mix.004).
...
def build_slice2bu_scene(
    image_path="/tmp/qt_slice2bu_env.exr",
    *,
    mode="claim",
):
    ...
    mode:
      ...
      refuse_noise     — ColorRamp.Fac←Noise named REFUSE Slice 2bu
      refuse_add       — Mix.004 ← Add(Mix, Glossy) named REFUSE Slice 2bu
      refuse_third     — third Mix hop named REFUSE Slice 2bu
      refuse_linked    — Glass.Color linked REFUSE Slice 2bu
    """
    ...
    if mode_key not in allowed:
        raise RuntimeError(f"mode={mode!r} refused (Slice 2bu)")
    ...
    def _mk_translucent(color=(0.036, 0.036, 0.036, 1.0)):
        tl = nt.nodes.new("ShaderNodeBsdfTranslucent")
        tl.inputs["Color"].default_value = color
        return tl

    def _mk_sss(color=(0.8, 0.8, 0.8, 1.0), scale=0.1):
        sss = nt.nodes.new("ShaderNodeSubsurfaceScattering")
        sss.inputs["Color"].default_value = color
        sss.inputs["Scale"].default_value = scale
        return sss

    def _wire_outer(deep_out):
        lp = nt.nodes.new("ShaderNodeLightPath")
        inner = nt.nodes.new("ShaderNodeMixShader")
        nt.links.new(lp.outputs["Is Shadow Ray"], _fac_sock(inner))
        nt.links.new(deep_out, _sh0(inner))
        nt.links.new(_mk_trans().outputs["BSDF"], _sh1(inner))
        outer = nt.nodes.new("ShaderNodeMixShader")
        nt.links.new(_loft_math_fac(), _fac_sock(outer))
        nt.links.new(inner.outputs["Shader"], _sh0(outer))
    ...
    if mode_key == "claim":
        # Mix-under-Add CLAIM (loft Mix.002 topology, unlinked leaves):
        # Mix.004 Fac<-ColorRamp(+MATH) Shader=Add(
        #   Mix(Fac unlinked 0.5, Glossy GGX + Transparent), Translucent)
        # Shader_001=Glass.
        ...
        _fac_sock(amix).default_value = 0.5
        nt.links.new(_mk_glossy().outputs["BSDF"], _sh0(amix))
        nt.links.new(_mk_trans().outputs["BSDF"], _sh1(amix))
        nt.links.new(amix.outputs["Shader"], a0)
        nt.links.new(_mk_translucent().outputs["BSDF"], a1)
        nt.links.new(add.outputs["Shader"], _sh0(deep))
        nt.links.new(_mk_glass().outputs["BSDF"], _sh1(deep))
        _wire_outer(deep.outputs["Shader"])
        bpy.context.view_layer.update()
        return scene, cube_obj, lamp, cam, img

    if mode_key == "add":
        # 2bt identity regression: Add(Glossy+Transparent)+Glass, no Mix under Add.
        deep = nt.nodes.new("ShaderNodeMixShader")
        ramp = _loft_colorramp(math_fac=True)
        nt.links.new(ramp.outputs["Color"], _fac_sock(deep))
        add = nt.nodes.new("ShaderNodeAddShader")
    ...
    if mode_key == "refuse_colorramp":
        # Mix under Add Fac<-ColorRamp (loft Mix.002 Fac leftover)
        cr = nt.nodes.new("ShaderNodeValToRGB")
        amix = nt.nodes.new("ShaderNodeMixShader")
        nt.links.new(cr.outputs["Color"], _fac_sock(amix))
        nt.links.new(_mk_glossy().outputs["BSDF"], _sh0(amix))
        nt.links.new(_mk_trans().outputs["BSDF"], _sh1(amix))
        add = nt.nodes.new("ShaderNodeAddShader")
        a0, a1 = _add_socks(add)
    ...
    if mode_key == "refuse_nested":
        # Mix under Add with nested Mix leaf (loft Mix.001 under Mix.002)
        nested = nt.nodes.new("ShaderNodeMixShader")
        _fac_sock(nested).default_value = 0.1
        nt.links.new(_mk_glass().outputs["BSDF"], _sh0(nested))
        nt.links.new(_mk_trans().outputs["BSDF"], _sh1(nested))
        amix = nt.nodes.new("ShaderNodeMixShader")
        _fac_sock(amix).default_value = 0.5
        nt.links.new(nested.outputs["Shader"], _sh0(amix))
        nt.links.new(_mk_glossy().outputs["BSDF"], _sh1(amix))
        add = nt.nodes.new("ShaderNodeAddShader")
        a0, a1 = _add_socks(add)
        nt.links.new(amix.outputs["Shader"], a0)
        nt.links.new(_mk_translucent().outputs["BSDF"], a1)
    ...
    if mode_key == "refuse_linked_rough":
        amix = nt.nodes.new("ShaderNodeMixShader")
        _fac_sock(amix).default_value = 0.5
        glossy = _mk_glossy()
        mathn = nt.nodes.new("ShaderNodeMath")
        mathn.operation = "ADD"
        mathn.inputs[0].default_value = 0.05
        mathn.inputs[1].default_value = 0.0
        nt.links.new(mathn.outputs["Value"], glossy.inputs["Roughness"])
        nt.links.new(glossy.outputs["BSDF"], _sh0(amix))
        nt.links.new(_mk_trans().outputs["BSDF"], _sh1(amix))
        add = nt.nodes.new("ShaderNodeAddShader")
        a0, a1 = _add_socks(add)
        nt.links.new(amix.outputs["Shader"], a0)
    ...
    if mode_key == "refuse_refraction":
        amix = nt.nodes.new("ShaderNodeMixShader")
        _fac_sock(amix).default_value = 0.5
        ref = nt.nodes.new("ShaderNodeBsdfRefraction")
        nt.links.new(ref.outputs["BSDF"], _sh0(amix))
        nt.links.new(_mk_glossy().outputs["BSDF"], _sh1(amix))
        add = nt.nodes.new("ShaderNodeAddShader")
        a0, a1 = _add_socks(add)
        nt.links.new(amix.outputs["Shader"], a0)
    ...
    if mode_key == "refuse_sss_ramp":
        # Mix.003 leftover: SSS Color<-ColorRamp
        amix = nt.nodes.new("ShaderNodeMixShader")
        _fac_sock(amix).default_value = 0.11
        sss = _mk_sss()
        cr = nt.nodes.new("ShaderNodeValToRGB")
        nt.links.new(cr.outputs["Color"], sss.inputs["Color"])
        nt.links.new(sss.outputs["BSSRDF"], _sh0(amix))
        nt.links.new(_mk_translucent().outputs["BSDF"], _sh1(amix))
        add = nt.nodes.new("ShaderNodeAddShader")
        a0, a1 = _add_socks(add)
        nt.links.new(amix.outputs["Shader"], a0)
        nt.links.new(_mk_glossy().outputs["BSDF"], a1)
        deep = nt.nodes.new("ShaderNodeMixShader")
        _fac_sock(deep).default_value = 0.35
        nt.links.new(add.outputs["Shader"], _sh0(deep))
        nt.links.new(_mk_glass().outputs["BSDF"], _sh1(deep))
...
def main():
    ...
    build_slice2bu_scene(image_path=args.image, mode=args.mode)
    print("QUANTTRACE_SLICE2BU_SCENE", args.mode)
