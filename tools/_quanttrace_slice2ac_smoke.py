# pack_scene Environment Texture Vector → Session vs stock Cycles (Slice 2ac).
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2ac_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2ac_stock.exr")
    ...
    if args.render_stock:
        ...
        print(
            "QUANTTRACE_SLICE2AC_STOCK wall", round(time.perf_counter() - t_s, 3),
            "out", args.stock_out,
        )
        if not args.compare:
    ...
    print(
        "QUANTTRACE_SLICE2AC_SMOKE packed",
    ...
    print(
        "QUANTTRACE_SLICE2AC_CTYPES path", desc.world_image_path,
        "mode", desc.world_tex_vector_mode,
        "map_rot", list(desc.world_map_rotation),
        "ver", ver,
    )
    ...
    print(
        "QUANTTRACE_SLICE2AC_SMOKE rc", rc, "wall", round(time.perf_counter() - t0, 3),
    ...
    if constant:
        raise RuntimeError("session Combined constant — env/vector likely dead")
    if args.compare:
        if os.path.isfile(args.compare):
            try:
                ...
                print(
                    "QUANTTRACE_SLICE2AC_STOCK_VIS min", smin, "max", smax,
                ...
                print("QUANTTRACE_SLICE2AC_STOCK_VIS skip (no OIIO in this process)")
        ...
        print("QUANTTRACE_SLICE2AC_SMOKE compare rc", rcode)
        if rcode != 0:
            raise SystemExit(rcode)
    print("QUANTTRACE_SLICE2AC_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2AC_SMOKE FAIL", type(e).__name__, e)
        raise
