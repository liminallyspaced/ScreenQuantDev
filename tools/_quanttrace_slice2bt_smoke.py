# pack_scene Mix Glass+Transparent -> Session vs stock (Slice 2bt).
...
def _compose_pair_png(stock_path, session_path, out_paths):
    ...
    for path in out_paths:
        ...
        out.close()
        print("QUANTTRACE_SLICE2BT_PAIR", path, "wh", w, h)


def _loft_pack_probe(qt_sync, qt_engine, loft_path):
    if not os.path.isfile(loft_path):
        print("QUANTTRACE_SLICE2BT_SMOKE loft MISSING", loft_path)
        print("PACK_FAIL loft file missing")
        return 1
    print("QUANTTRACE_SLICE2BT_SMOKE loft open", loft_path)
    ...
    try:
        ...
        print("QUANTTRACE_SLICE2BT_SMOKE loft PACK_OK (no dmax claim)")
        return 0
    except qt_sync.QuantTraceSyncError as e:
        msg = str(e)
        print("PACK_FAIL", msg)
        print("QUANTTRACE_SLICE2BT_SMOKE loft PACK_FAIL", type(e).__name__, msg)
        return 0
...
def main():
    ...
    p.add_argument("--out", default="/tmp/quanttrace_slice2bt_session.exr")
    p.add_argument("--stock-out", default="/tmp/quanttrace_slice2bt_stock.exr")
    ...
    import _quanttrace_slice2bt_scene as sc2bo
    scene, cube_obj, lamp, cam, img = sc2bo.build_slice2bt_scene(
    ...
    try:
        ...
        msg = str(e)
        print("QUANTTRACE_SLICE2BT_PACK_REFUSE", msg)
        if refuse:
            if "Slice 2bt" in msg or "Slice 2bs" in msg:
                print("QUANTTRACE_SLICE2BT_SMOKE REFUSE_OK", msg)
                return 0
            raise RuntimeError(f"refuse missing Slice 2bt/2bs tag: {msg}")
        raise

    if refuse:
        raise RuntimeError("expected Slice 2bt refuse but pack_scene succeeded")
    ...
    if args.mode == "claim":
        ...
        if addk != {3, 1}:
            raise RuntimeError(f"claim Add children not Glossy+Transparent: {addk}")
        if abs(float(m0.get("mix_nested2_glossy_roughness", -1)) - 0.05) > 1e-6:
            raise RuntimeError(f"claim Glossy roughness not 0.05: {m0.get('mix_nested2_glossy_roughness')}")
        ...
        if int(m0.get("mix_nested2_ramp_n", 0) if m0.get("mix_nested2_ramp_n") is not None else 0) <= 0:
            raise RuntimeError("claim nested2 ramp_n should be >0")
        if int(m0.get("mix_nested2_lightpath_enable", 0) if m0.get("mix_nested2_lightpath_enable") is not None else 0) != 0:
            raise RuntimeError("claim nested2 Fac should not be LightPath when ramp on")
        ...
        if int(m0.get("mix_nested2_ramp_math_a_kind", -1) if m0.get("mix_nested2_ramp_math_a_kind") is not None else -1) != 3:
            raise RuntimeError("claim ramp math A should be NEW_GEOMETRY Backfacing")
        if int(m0.get("mix_nested2_ramp_math_b_kind", -1) if m0.get("mix_nested2_ramp_math_b_kind") is not None else -1) != 4:
            raise RuntimeError("claim ramp math B should be HueSat")
        if abs(float(m0.get("mix_nested2_ramp_hsv_val", 0.0)) - 2.0) > 1e-6:
            raise RuntimeError(f"claim HueSat.Value not 2.0: {m0.get('mix_nested2_ramp_hsv_val')}")
        if int(m0.get("mix_nested2_ramp_hsv_color_kind", -1) if m0.get("mix_nested2_ramp_hsv_color_kind") is not None else -1) != 1:
            raise RuntimeError("claim HueSat.Color should be LightPath")
        if int(m0.get("mix_nested2_ramp_hsv_color_lightpath", -1) if m0.get("mix_nested2_ramp_hsv_color_lightpath") is not None else -1) != 7:
            raise RuntimeError("claim HueSat.Color should be Ray Length")
    elif args.mode == "ramp":
    ...
    if args.force_pure_glass:
        for m in packed["meshes"]:
            # 2bt-bypass: Add off -> nested2 Glass+Transparent (2bs path).
            m["mix_nested2_add_enable"] = 0
            m["mix_nested2_closure1_kind"] = 0
            m["mix_nested2_closure2_kind"] = 1
        print("QUANTTRACE_SLICE2BT_FORCE_ADD_BYPASS")
    ...
    wall = time.perf_counter() - t0
    print(
        "QUANTTRACE_SLICE2BT_SMOKE rc", rc, "wall", round(wall, 3),
        "ver", ver, "is_tracer", is_tr,
    ...
    if args.render_stock:
        ...
        print("QUANTTRACE_SLICE2BT_STOCK wrote", args.stock_out)
        if not args.compare:
    ...
    if args.compare:
        ...
        print("QUANTTRACE_SLICE2BT_SMOKE compare rc", rcode)
        if args.pair_png and rcode == 0:
            pair = args.pair_png
            copies = [x.strip() for x in pair.split(",") if x.strip()] or [pair]
            proof = os.path.join(root, "docs", "proof", "quanttrace-add-glossy-32-pair.png")
            ws = "/workspace/quanttrace-add-glossy-32-pair.png"
            ...
            try:
                ...
                print("QUANTTRACE_SLICE2BT_PAIR FAIL", type(e).__name__, e)
                raise
        if rcode != 0:
            raise SystemExit(rcode)

    print("QUANTTRACE_SLICE2BT_SMOKE OK")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("QUANTTRACE_SLICE2BT_SMOKE FAIL", type(e).__name__, e)
        raise
