...
**Current native:** `0.0.62-slice2bi` — Normal Map Color ← Combine+InvertG Separate←TEX_IMAGE. Addon still `0.3.3`.
...
`-DQT_WITH_CYCLES=ON`. Native `0.0.62-slice2bi` (Normal Map Color ← Combine+InvertG).
...
| **2bi — Normal Map Color ← Combine+InvertG** | **PASS** | invert-G CLAIM 32²/4 Δmax **7.15e-7**; 256²/128 Δmax **5.36e-7**. normal 2j/mix 2ay/curves 2bh/hdr 32²/4 PASS. Loft Plane.002 Rope cleared; PACK_FAIL Chocofur_Free_Sideboard_01 Roughness←SEPARATE_COLOR. Native `0.0.62-slice2bi`. |
